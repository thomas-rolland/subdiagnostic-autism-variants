...
# - regressions_brain_anatomy.pdf = Plot of betas, 95% CI and corrected p-value for regressions on cortical and subcortical traits
#===================================================================================================
...
for i in range(0, len(autismOR_thresholds) - 1):
	...
	for k in range(0, len(model_fit.params)):
...
regressions.loc[(regressions["feature"].isin(pd.Series(["autism_status", "scq", "F1", "F2", "F3", "F4", "F5", "F6", "IQ_score"])) == True), "Padj"] = sm.multitest.multipletests(list(regressions["P"][(regressions["feature"].isin(pd.Series(["autism_status", "scq", "F1", "F2", "F3", "F4", "F5", "F6", "IQ_score"])) == True)]), method = "fdr_bh", alpha = 0.05)[1]
regressions.loc[(regressions["feature"].isin(pd.Series(["walked", "smiled", "words", "crawled", "sat", "combined_words"])) == True), "Padj"] = sm.multitest.multipletests(list(regressions["P"][(regressions["feature"].isin(pd.Series(["walked", "smiled", "words", "crawled", "sat", "combined_words"])) == True)]), method = "fdr_bh", alpha = 0.05)[1]
regressions.loc[(regressions["feature"].isin(pd.Series(["income", "qualification", "townsend_index", "fluid_intelligence"])) == True), "Padj"] = sm.multitest.multipletests(list(regressions["P"][(regressions["feature"].isin(pd.Series(["income", "qualification", "townsend_index", "fluid_intelligence"])) == True)]), method = "fdr_bh", alpha = 0.05)[1]
...
################################################################################################################################################
########################################## Performing regressions on cortical and subcortical metrics ##########################################
################################################################################################################################################
...
regressions = pd.DataFrame()
columns = [col for col in individuals_tmp.columns if '_z_scored' in col]
for feature in columns:
    model_fit = ols(feature + ' ~ C(has_variant) * ASD_PRS + C(sex) + age + has_variant:age + has_variant:C(sex) + C(sex):age + age_sq + C(SITE)', data=individuals_tmp).fit()
    ci = model_fit.conf_int(alpha=0.05, cols=None)
    for j in range(0, len(model_fit.params)):
...
print (">>>>>>>>>>> Correcting p-values for multiple testing...")
regressions["Padj"] = sm.multitest.multipletests(list(regressions["P"]), method = "fdr_bh", alpha = 0.05)[1]
